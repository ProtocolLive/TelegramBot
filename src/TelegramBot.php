...
class TelegramBot extends TelegramBot_Basics{
  ...
  private function ParseServer(array $Server){
    ...
    elseif(isset($Server['message']['text'])):
      ...
      $this->Server->Event->Reply = $Server['message']['reply_to_message']['message_id'] ?? null;
    ...
    elseif(isset($Server['callback_query'])):
      ...
      $this->Server->Event->CallbackId = $Server['callback_query']['id'];
    ...
    elseif(isset($Server['edited_message'])):
      $this->Server->Event = new TelegramBot_FactoryEventText;
      $this->Server->Event->Type = TelegramBot_Basics::Event_Edited;
      $this->Server->Event->Reply = $Server['edited_message']['reply_to_message']['message_id'] ?? null;
      $this->Server->Event->Msg = $Server['edited_message']['text'];
      $this->Server->Event->Id = $Server['edited_message']['message_id'];
      $this->ParseUser($Server['edited_message']);
      $this->ParseChat($Server['edited_message']);
    endif;
  }
...
// ------------------------ Get / Set -----------------------------
  ...
  public function CallbackId():?string{
    if($this->Server->Event->Type === self::Event_Callback):
      return $this->Server->Event->CallbackId;
    else:
